<script lang="ts">
  ...
  let driftedResources = $derived(resources.filter((item) => item.desired_state && item.observed_state && JSON.stringify(item.desired_state.state) !== JSON.stringify(item.observed_state.state)));
  let runningOperations = $derived(operations.filter((item) => ['queued', 'running', 'retrying'].includes(item.status)));
  ...
  onMount(async () => {
    ...
    try {
      ...
      token = session.token;
      await initialize();
    } catch { sessionStorage.removeItem('multicloud.session'); token = ''; }
  });
  ...
  async function initialize() {
    ...
    try {
      ...
      organizationId = organizations.some((item) => item.id === preferred) ? (preferred ?? '') : (organizations[0]?.id ?? '');
      if (organizationId) { client.setOrganization(organizationId); await refreshAll(); }
    } catch (cause) { if (cause instanceof ApiError && cause.status === 401) logout(); else error = messageOf(cause); }
    finally { loading = false; }
  }

  async function changeOrganization() {
    if (!client || !organizationId) return;
    localStorage.setItem('multicloud.organization', organizationId); client.setOrganization(organizationId); selectedResource = null; await refreshAll();
  }
  ...
  async function createProvider() {
    if (!client) return;
    savingProvider = true; error = '';
    try {
      let credential: Record<string, unknown>;
      if (providerKind === 'cloudflare' && useGlobalKey) credential = { credential_type: 'global_api_key', email: emailIdentity, global_api_key: globalApiKey };
      else if (providerKind === 'ovh') credential = { credential_type: 'ovh_application', application_key: applicationKey, application_secret: applicationSecret, consumer_key: consumerKey };
      else credential = { credential_type: 'api_token', api_token: apiToken };
      await client.createProvider({ provider_kind: providerKind, name: providerName, configuration: {}, ...credential });
      providerDialog = false; clearCredentialForm(); notice = 'Provider account encrypted and ready for validation.'; await refreshAll();
    } catch (cause) { error = messageOf(cause); }
    finally { savingProvider = false; }
  }
  function clearCredentialForm() { providerName = ''; apiToken = ''; emailIdentity = ''; globalApiKey = ''; applicationKey = ''; applicationSecret = ''; consumerKey = ''; useGlobalKey = false; }

  async function testConnection(provider: ProviderAccount) {
    if (!client) return; actionBusy = provider.id;
    try { const result = await client.testProvider(provider.id); notice = result.valid ? `${provider.name} connection verified.` : `${provider.name} validation failed.`; await refreshAll(); }
    catch (cause) { error = messageOf(cause); } finally { actionBusy = ''; }
  }
  async function syncProvider(provider: ProviderAccount) {
    if (!client) return; actionBusy = provider.id;
    try { await client.syncProvider(provider.id, provider.provider_kind === 'cloudflare' ? 'dns_zone' : 'compute_instance'); notice = `${provider.name} inventory sync queued.`; await refreshAll(); }
    catch (cause) { error = messageOf(cause); } finally { actionBusy = ''; }
  }
  async function openResource(resource: Resource) {
    if (!client) return; selectedResource = resource; detailLoading = true;
    try { [resourceDrifts, reconciliations] = await Promise.all([client.drifts(resource.id), client.reconciliations(resource.id)]); }
    catch (cause) { error = messageOf(cause); } finally { detailLoading = false; }
  }
  ...
  async function lifecycle(resource: Resource, action: string) {
    if (!client) return;
    const provider = resourceProvider(resource), externalId = resource.external_id ?? '';
    if (!provider || !externalId) { error = 'This resource does not expose an operable provider mapping yet.'; return; }
    actionBusy = `${resource.id}:${action}`;
    try { await client.runProviderOperation(provider.id, action, externalId); notice = `${action} operation queued for ${resource.name}.`; await refreshAll(); }
    catch (cause) { error = messageOf(cause); } finally { actionBusy = ''; }
  }
  async function approve(task: Reconciliation) {
    if (!client || !selectedResource) return; actionBusy = task.id;
    try { await client.approveReconciliation(selectedResource.id, task.id); notice = 'Reconciliation approved.'; await openResource(selectedResource); }
    catch (cause) { error = messageOf(cause); } finally { actionBusy = ''; }
  }
  async function cancel(operation: Operation) {
    if (!client) return; actionBusy = operation.id;
...
<svelte:head><title>MultiCloud · Command Center</title><meta name="description" content="Multi-tenant cloud operations control plane" /></svelte:head>
...
{:else}
  <div class="app-shell">
    <aside class:open={mobileNav}><div class="brand sidebar-brand"><span class="brand-glyph">M</span><span>MultiCloud</span></div><div class="tenant-chip"><span class="tenant-avatar">{activeOrganization?.name.slice(0, 2).toUpperCase() ?? '--'}</span><div><small>ACTIVE ORGANIZATION</small><strong>{activeOrganization?.name ?? 'Select tenant'}</strong></div></div><nav aria-label="Primary navigation">{#each navigation as item}<button class:active={view === item.id} onclick={() => { view = item.id; mobileNav = false; }}><span class="nav-icon">{item.icon}</span><span><strong>{item.label}</strong><small>{item.caption}</small></span></button>{/each}</nav><div class="sidebar-foot"><div class="system-health"><i></i><span><strong>Control plane</strong><small>All systems nominal</small></span></div><button class="text-button" onclick={logout}>Sign out</button></div></aside>
    <main class="workspace"><header class="topbar"><button class="menu-button" aria-label="Toggle navigation" onclick={() => mobileNav = !mobileNav}>☰</button><div><p class="breadcrumb">CONTROL PLANE / <span>{view.toUpperCase()}</span></p><h1>{navigation.find((item) => item.id === view)?.label}</h1></div><div class="top-actions"><select aria-label="Organization" bind:value={organizationId} onchange={changeOrganization}>{#each organizations as organization}<option value={organization.id}>{organization.name}</option>{/each}</select><button class="icon-button" aria-label="Refresh data" onclick={refreshAll} disabled={loading}>↻</button><span class="operator-avatar">OP</span></div></header>
      ...
      <div class="content" class:loading>
        ...
        {:else if view === 'overview'}
          ...
          <section class="dashboard-grid"><article class="panel span-two"><div class="panel-head"><div><p class="kicker">OPERATION TELEMETRY</p><h3>Execution stream</h3></div><button class="text-button" onclick={() => view = 'operations'}>View all →</button></div><div class="timeline-chart"><div class="chart-grid"></div><svg viewBox="0 0 800 180" preserveAspectRatio="none"><path d="M0 145 C70 130 90 135 150 105 S250 135 315 80 S420 115 485 55 S590 90 650 38 S750 55 800 20" /></svg><span>Time-series telemetry connects in Observability phase</span></div></article><article class="panel"><div class="panel-head"><div><p class="kicker">PROVIDER FABRIC</p><h3>Connection status</h3></div></div><div class="provider-mini-list">{#each providers.slice(0, 4) as provider}<button onclick={() => view = 'providers'}><span class="provider-logo {provider.provider_kind}">{provider.provider_kind.slice(0, 2).toUpperCase()}</span><span><strong>{provider.name}</strong><small>{provider.provider_kind}</small></span><i class:online={provider.status === 'active'}></i></button>{:else}<div class="compact-empty">Connect a provider to activate the fabric.</div>{/each}</div></article>
            <article class="panel span-two"><div class="panel-head"><div><p class="kicker">RECENT ACTIVITY</p><h3>Operation ledger</h3></div></div><div class="table-wrap"><table><thead><tr><th>Operation</th><th>Target</th><th>Status</th><th>Progress</th><th>Created</th></tr></thead><tbody>{#each operations.slice(0, 5) as operation}<tr><td><strong>{operation.operation_type}</strong><small>{shortId(operation.id)}</small></td><td>{operation.target_type}</td><td><span class="status {operation.status}">{operation.status}</span></td><td><div class="progress"><i style={`width:${operation.progress}%`}></i></div></td><td>{relativeDate(operation.created_at)}</td></tr>{:else}<tr><td colspan="5" class="table-empty">No operations recorded yet.</td></tr>{/each}</tbody></table></div></article><article class="panel"><div class="panel-head"><div><p class="kicker">COST SIGNAL</p><h3>Cloud spend</h3></div><span class="beta">PHASE 9</span></div><div class="future-metric"><span>—</span><p>Billing telemetry is reserved for the Billing bounded context.</p><div class="ghost-bars"><i></i><i></i><i></i><i></i><i></i><i></i></div></div></article></section>
  ...
  {#if providerDialog}<div class="modal-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && (providerDialog = false)}><form class="modal" onsubmit={(event) => { event.preventDefault(); createProvider(); }}><div class="modal-head"><div><p class="kicker">NEW CONNECTION</p><h2>Connect provider</h2></div><button type="button" onclick={() => providerDialog = false}>×</button></div><label>Provider<select bind:value={providerKind}><option value="cloudflare">Cloudflare · DNS</option><option value="vultr">Vultr · Compute</option><option value="ovh">OVHcloud · VPS</option></select></label><label>Connection name<input bind:value={providerName} placeholder="Production account" required /></label>
    {#if providerKind === 'cloudflare'}<div class="credential-choice"><button type="button" class:active={!useGlobalKey} onclick={() => useGlobalKey = false}><strong>API Token</strong><small>✓ Recommended · Restricted scope</small></button><button type="button" class:risk={useGlobalKey} onclick={() => useGlobalKey = true}><strong>Global API Key</strong><small>⚠ Legacy · Full account access</small></button></div>{#if useGlobalKey}<div class="risk-banner"><strong>High-risk credential</strong><span>Use a scoped API Token whenever possible. This action is security-audited.</span></div><label>Cloudflare email<input bind:value={emailIdentity} type="email" required /></label><label>Global API Key<input bind:value={globalApiKey} type="password" required /></label>{:else}<label>API Token<input bind:value={apiToken} type="password" autocomplete="off" required /><small>Token should include Zone Read and DNS Edit scopes.</small></label>{/if}
    {:else if providerKind === 'vultr'}<label>API Token<input bind:value={apiToken} type="password" autocomplete="off" required /><small>Use a dedicated token with minimum Compute permissions.</small></label>
    {:else}<div class="info-banner">OVHcloud signs each request with three encrypted credential components.</div><label>Application Key<input bind:value={applicationKey} autocomplete="off" required /></label><label>Application Secret<input bind:value={applicationSecret} type="password" autocomplete="off" required /></label><label>Consumer Key<input bind:value={consumerKey} type="password" autocomplete="off" required /></label>{/if}
    <div class="modal-actions"><button type="button" onclick={() => providerDialog = false}>Cancel</button><button class="primary" disabled={savingProvider}>{savingProvider ? 'Encrypting…' : 'Encrypt & connect'}</button></div></form></div>{/if}

  {#if selectedResource}<div class="drawer-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && (selectedResource = null)}><aside class="drawer"><div class="modal-head"><div><p class="kicker">RESOURCE DETAIL</p><h2>{selectedResource.name}</h2></div><button onclick={() => selectedResource = null}>×</button></div><div class="resource-identity"><span class="provider-logo large">{selectedResource.resource_type === 'compute_instance' ? 'VM' : 'DN'}</span><div><span class="status {selectedResource.lifecycle}">{selectedResource.lifecycle}</span><p>{selectedResource.resource_type.replaceAll('_', ' ')} · {selectedResource.region ?? 'global'}</p></div></div>{#if selectedResource.resource_type === 'compute_instance'}<div class="action-strip">{#each ['start', 'stop', 'reboot'] as action}<button onclick={() => selectedResource && lifecycle(selectedResource, action)} disabled={actionBusy === `${selectedResource.id}:${action}`}>{action === 'start' ? '▶' : action === 'stop' ? '■' : '↻'} {action}</button>{/each}</div>{/if}<section class="drawer-section"><div class="panel-head"><h3>Observed state</h3><span class="beta">v{selectedResource.observed_state?.version ?? 0}</span></div><pre>{JSON.stringify(selectedResource.observed_state?.state ?? selectedResource.attributes, null, 2)}</pre></section><section class="drawer-section"><div class="panel-head"><h3>Configuration drift</h3><span>{resourceDrifts.length}</span></div>{#if detailLoading}<p class="muted">Loading state analysis…</p>{:else if resourceDrifts.length}{#each resourceDrifts as drift}<div class="drift-item"><span class="status {drift.status}">{drift.status}</span><div><strong>{Object.keys(drift.differences).length} managed differences</strong><small>{relativeDate(drift.detected_at)}</small></div></div>{/each}{:else}<div class="aligned-state"><span>✓</span><div><strong>No drift detected</strong><small>Observed and desired fields are aligned.</small></div></div>{/if}</section><section class="drawer-section"><div class="panel-head"><h3>Reconciliation</h3><span>{reconciliations.length}</span></div>{#each reconciliations as task}<div class="task-item"><div><span class="status {task.status}">{task.status}</span><strong>{task.policy.replaceAll('_', ' ')}</strong></div>{#if task.status === 'pending' && task.policy === 'manual_approval'}<button class="accent" onclick={() => approve(task)} disabled={actionBusy === task.id}>Approve</button>{/if}</div>{:else}<p class="muted">No reconciliation tasks.</p>{/each}</section></aside></div>{/if}
{/if}
