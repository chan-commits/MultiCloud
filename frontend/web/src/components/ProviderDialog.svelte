  ...
  >
    <div class="flex items-start justify-between border-b border-[#192a36] pb-[17px]">
      <div>
        ...
        <h2 class="m-0 text-[30px] tracking-[-0.035em] text-[#f2f8ff]">{t('Connect provider')}</h2>
      </div>
      <button
    ...
    <label
      >{t('Provider')}<select bind:value={providerKind}
    ...
    {#if providerKind === 'cloudflare'}<div class="grid grid-cols-2 gap-2 max-[760px]:grid-cols-1">
        <button
          ...
          ><strong>API Token</strong><small>✓ {t('Recommended')} · {t('Restricted scope')}</small
          ></button
        ><button
          ...
          ><strong>Global API Key</strong><small>⚠ {t('Legacy')} · {t('Full account access')}</small
          ></button
        >
      </div>
      {#if useGlobalKey}<div
        ...
        >
          <strong>{t('High-risk credential')}</strong><span
            >{t('Use a scoped API Token whenever possible. This action is security-audited.')}</span
          >
        </div>
        <label
          >{t('Cloudflare email')}<input bind:value={emailIdentity} type="email" required /></label
        ...
        >{:else}<label
          >API Token<input
            ...
            autocomplete="off"
            required
          /><small>{t('Token should include Zone Read and DNS Edit scopes.')}</small></label
        >{/if}
    {:else if providerKind === 'vultr'}<label
        >API Token<input bind:value={apiToken} type="password" autocomplete="off" required /><small
          >{t('Use a dedicated token with minimum Compute permissions.')}</small
        ></label
      >
    {:else}<div class="border border-[#1c5862] bg-[#0d242a] p-[11px] text-[10px] text-[#68ccd3]">
        {t('OVHcloud signs each request with three encrypted credential components.')}
      </div>
      <label
        >{t('Application Key')}<input
          bind:value={applicationKey}
          autocomplete="off"
          required
        /></label
      ><label
        >{t('Application Secret')}<input
    ...
    <div class="flex justify-end gap-[9px] pt-2">
      ...
      ><button
        ...
        disabled={saving}>{t(saving ? 'Encrypting…' : 'Encrypt & connect')}</button
      >
    </div>
  </form>
</div>
